...
/**
 * Find citations in string stream and convert it to object mode stream
 * @type {ParserTransform}
 */
module.exports = class ParserTransform extends Transform {
   ...
   * @param {Object} [options.normalize] Object with functions for normalizing the matched citations
   */
  constructor(options) {
    super({
      objectMode: true,
  ...
  _transform (obj, encoding, cb) {
    ...
    Object.keys(count_match).map((match) => {
      ...
      if (typeof citations === 'string') {
        ...
        console.log('citations is array', citations);
        let citation;
        for (let i = 0, ii = citations.length; i < ii; i++) {
          citation = citations[i];
          references[citation] = positions[match];
          console.log('citation', i, citation, positions[match]);
        }
      }
  ...
  _normalize(match) {
    ...
    Object.keys(this.normalize).map((stage_name) => {
      ...
      if (Array.isArray( results )) {
        let temp_results = [];
        results.map((item) => {
          console.log('_normalize - results.map', item );
          let temp = stage_func( item );
          console.log('_normalize - temp', temp );
          temp_results.push( temp );
        });
        console.log('_normalize - isArray( results ) before results', results );
        results = temp_results;
        console.log('_normalize - isArray( results ) after results', results );
      } else {
    ...
    console.log('_normalize - final results', results);
